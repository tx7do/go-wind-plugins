...
func (pka *Authenticator) CreateIdentityWithContext(ctx context.Context, claims engine.AuthClaims) (context.Context, error) {
	token, err := pka.CreateIdentity(claims)
	if err != nil {
		return ctx, err
	}
	ctx = engine.MDWithAuth(ctx, engine.BearerWord, token)
	return ctx, nil
}
