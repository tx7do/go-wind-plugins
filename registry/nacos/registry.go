...
var ErrServiceInstanceNameEmpty = errors.New("wind/nacos: ServiceInstance.Name can not be empty")
...
func (r *Registry) Register(_ context.Context, si *wind.Instance) error {
	if si.Name == "" {
...
func (r *Registry) Deregister(_ context.Context, service *wind.Instance) error {
...
func (r *Registry) GetService(_ context.Context, serviceName string) ([]*wind.Instance, error) {
	...
	items := make([]*wind.Instance, 0, len(res))
	for _, in := range res {
		kind := r.opts.kind
		weight := r.opts.weight
